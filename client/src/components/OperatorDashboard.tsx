...
export function OperatorDashboard({ user }: OperatorDashboardProps) {
  ...
  const stats = [
    { title: t('myTasks'), value: tasks.length.toString(), icon: CheckSquare, color: 'bg-blue-500' },
    { title: t('inProgress'), value: tasks.filter(t => t.status === 'In Progress').length.toString(), icon: Play, color: 'bg-orange-500' },
    { title: t('completed'), value: tasks.filter(t => t.status === 'Completed').length.toString(), icon: CheckCircle, color: 'bg-green-500' },
    { title: t('overdue'), value: overdueTasks.length.toString(), icon: AlertTriangle, color: 'bg-red-500' }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        <h2 className="text-2xl font-bold text-slate-900 mb-2">{t('welcome')}, {user.name}!</h2>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('myAssignedTasks')}</CardTitle>
          <CardDescription>{t('viewAndUpdateTasks')}</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-center text-slate-500 py-8">{t('noTasksAssigned')}</p>
          ) : (
                  ...
                  <div key={task._id} className={`border rounded-lg p-4 ${isOverdue ? 'border-red-200 bg-red-50' : 'border-slate-200'}`}>
                    ...
                    {completingTask?._id === task._id && !isUpdating && (
                      <div className="mt-3 flex flex-col space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
                        <textarea
                          placeholder={t('addClosingComment')}
                          value={comment}
                    ...
                    <p className="text-xs text-slate-500">{t('assignedBy')}: {task.createdBy?.name}</p>
                    <div className="flex items-center space-x-2 mt-2">
                      <Badge className={getPriorityColor(task.priority)}>{task.priority}</Badge>
                      {isOverdue && <Badge className="bg-red-100 text-red-800">{t('overdue')}</Badge>}
                    </div>
                  </div>
                );
              })}
