...
export default function NotificationBell() {
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      console.error(error);
      alert(t('failedToClearNotifications'));
    }
  };
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-80 p-0">
        ...
        <div className="max-h-80 overflow-y-auto">
          {isLoading ? (
            <p className="p-4 text-sm text-center text-slate-500">{t('loading')}</p>
            ...
            <div className="divide-y">
              {notifications.map(n => (
                <div
                  key={n._id}
                  className={`p-3 cursor-pointer ${!n.read ? 'bg-blue-50 border-l-2 border-blue-500' : 'bg-white'
                    }`}
                >
